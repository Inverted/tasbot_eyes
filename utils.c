...
/**
 * Allocate memory and create a new string with given capacity
 * @param _cap The capacity for this string
 * @return The pointer to this new allocated string
 */
string_t* alloccstr(size_t _cap) {
    ...
    if (!s) {
        string_t message;
        initstr(&message, "[ERROR] alloccstr: Failed to allocate memory for string");
        failExit(&message);
    }
    ...
    //Allocate memory for string content based on determined length
    char* buffer = (char*) malloc(_cap * sizeof(char));
    if (!buffer) {
        string_t message;
        initstr(&message, "[ERROR] alloccstr: Failed to allocate memory for string buffer");
        failExit(&message);
    }
